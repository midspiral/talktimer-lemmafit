...
module TalkTimer refines Domain {
  ...
  ghost predicate LapsValid(laps: seq<Lap>) {
    forall i | 0 <= i < |laps| ::
      laps[i].timestamp >= 0 && laps[i].duration >= 0
  }
  ...
  function Apply(m: Model, a: Action): Model
  {
    match a
      case SetTime(ms) =>
        ...
        if ms >= m.currentTime then Model(ms, m.lastLapTime, m.laps) else m

      case CreateLap =>
        // Record lap with duration from last lap time
        var duration := m.currentTime - m.lastLapTime;
        var newLap := Lap(m.currentTime, duration, "", false);
        Model(m.currentTime, m.currentTime, m.laps + [newLap])

      case LabelLap(idx, name) =>
        if 0 <= idx < |m.laps| then
          Model(m.currentTime, m.lastLapTime,
                m.laps[idx := m.laps[idx].(section := name)])
        else
          m
      ...
      case SelectLap(idx) =>
        if 0 <= idx < |m.laps| then
          Model(m.currentTime, m.lastLapTime,
                m.laps[idx := m.laps[idx].(selected := !m.laps[idx].selected)])
        else
          m
      ...
      case DeleteLap(idx) =>
        ...
        if 0 <= idx < |m.laps| then
          Model(m.currentTime, m.lastLapTime, m.laps[..idx] + m.laps[idx+1..])
        else
          m
      ...
      case Reset =>
        // Clear all laps, reset lastLapTime to current time
